...
def extract_text_from_url(url):
    try:
        ...
        return content if content.strip() else "Error extracting content from the URL."
    except Exception:
...
def index():
    ...
    if request.method == "POST":
        ...
        # Get content either from direct text or URL
        if input_text:
            content_text = input_text
        elif url_input:
            content_text = extract_text_from_url(url_input)
        else:
            error_message = "⚠️ Please enter a statement or a valid URL."
            return render_template("index.html", error_message=error_message)

        # Check if content is valid
        if "Error extracting" in content_text or not content_text.strip():
            error_message = "⚠️ Could not extract valid content. Try a different URL or input."
            return render_template("index.html", error_message=error_message)
